...
Versão incrementada: moedas, inimigos, pontuação e vidas
...
pygame.display.set_caption("Mario 2D - Versão Incrementada")
...
try:
    ...
    # Falha ao inicializar som: ambiente pode não suportar áudio
...
def desenhar_hud(surface, jogador):
...
def main():
    ...
    while rodando:
        ...
        if not jogo_rodando:
            # apenas renderizar tela de game over
            janela.fill(COR_CEU)
            texto_go = large_font.render("GAME OVER", True, (200,20,20))
            sub = font.render("Pressione R para reiniciar", True, (0,0,0))
        ...
        desenhar_hud(janela, jogador)

        instrucoes = font.render("A/D ou ←/→: mover   Espaço: pular   Esc: sair", True, (0,0,0))
